...
To rerun the analysis, start with `nyt_main_training.py`, which trains the per-period Word2Vec models. `run_per_period_lda.py` and `run_lda_coherence.py` fit and score the topic models, and `rerun_lda_rf_option_a.py` runs the LDA plus Random Forest pipeline. The remaining scripts cover robustness checks (unified min_count, Procrustes alignment, bootstrap rank stability, concordance validation) and figure generation. `REPRODUCE.md` gives the full run order and maps each script to the tables and figures it produces.
